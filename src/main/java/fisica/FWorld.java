 ...
 * will have the size of the applet that it is created in. Once the world is
 * created we can add bodies to it or remove bodies we have added.
 *
 * Once the world is created we may add or remove bodies to it using
 * {@link #add(FBody) add} and {@link #remove(FBody) remove}. We may also call
 * {@link #step() step} to advance one step the simulation. Finally we can draw
 * the world and all the bodies living in it by using {@link #draw() draw}.
 *
 *
 *
 * <pre>
...
public class FWorld extends World {
	...
	class ConcreteContactListener implements ContactListener {
		public void add(ContactPoint point) {
			...
			try {
				function(m_world.m_contactStartedMethod).apply(Fisica.parent(), new Object[] { contact });
			} catch (Exception e) {
		...
		public void persist(ContactPoint point) {
			...
			try {
				function(m_world.m_contactPersistedMethod).apply(Fisica.parent(), new Object[] { contact });
			} catch (Exception e) {
		...
		public void remove(ContactPoint point) {
			...
			try {
				function(m_world.m_contactEndedMethod).apply(Fisica.parent(), new Object[] { contact });
			} catch (Exception e) {
		...
		public void result(ContactResult point) {
			...
			try {
				function(m_world.m_contactResultMethod).apply(Fisica.parent(), new Object[] { result });
			} catch (Exception e) {
	...
	public void dragBody(float x, float y) {
		...
		if (body.isStatic()) {
			body.setPosition(x - m_grabPositionX, y - m_grabPositionY);
		} else {
	...
	public FWorld(float topLeftX, float topLeftY, float bottomRightX, float bottomRightY) {
		super(new AABB(Fisica.screenToWorld(new Vec2(topLeftX, topLeftY)),
				Fisica.screenToWorld(new Vec2(bottomRightX, bottomRightY))),
		...
		m_contactStartedMethod = object(Fisica.parent()).$get("contactStarted");
		m_contactPersistedMethod = object(Fisica.parent()).$get("contactPersisted");
		m_contactEndedMethod = object(Fisica.parent()).$get("contactEnded");
		m_contactResultMethod = object(Fisica.parent()).$get("contactResult");
	...
	public void draw(PGraphics graphics) {
		processActions();
		/*
		 * for (Body b = getBodyList(); b != null; b = b.m_next) { FBody fb =
		 * (FBody)(b.m_userData); if (fb != null && fb.isDrawable()) fb.draw(applet); }
		 */
	...
	/**
	 * Draws all the bodies in the world on the applet canvas. This method is often
	 * called in the draw method of the applet.
	 *
	 * @see FBody
	 */
	public void draw() {
		draw(Fisica.parentGraphics());
	}

	/**
	 * Draws the debug version of all the bodies in the world on the applet canvas.
	 * This method is often called in the draw method of the applet.
	 *
	 * @see FBody
	 */
	public void drawDebug() {
		drawDebug(Fisica.parentGraphics());
	}
	 ...
	 *
	 * @param joint
	 *            joint to be added to the world
	 * @see #remove(FJoint)
	 */
	 ...
	 *
	 * @param joint
	 *            joint to be removed from the world
	 * @see #add(FJoint)
	 */
	...
	/**
	 * Add black edges of given dimensions to the world. This will create the bodies
	 * for {@link #left}, {@link #right}, {@link #bottom} and {@link #top}.
	 *
	 * @param topLeftX
	 *            the horizontal coordinate of the top left corner of the edges
	 * @param topLeftY
	...
	/**
	 * Add edges of given applet's dimensions to the world. This will create the
	 * bodies for {@link #left}, {@link #right}, {@link #bottom} and {@link #top}.
	 *
	 * @param applet
	 *            applet from where to get the dimensions for the edges
	 * @param color
	 *            the color of the edges. This color must be passed using
	 *            Processing's color() function
	 */
	...
	/**
	 * Add edges of Processing's canvas dimensions to the world. This will create
	 * the bodies for {@link #left}, {@link #right}, {@link #bottom} and
	 * {@link #top}.
	 *
	 * @param color
	 *            the color of the edges. This color must be passed using
	...
	/**
	 * Add black edges of Processing's canvas dimensions to the world. This will
	 * create the bodies for {@link #left}, {@link #right}, {@link #bottom} and
	 * {@link #top}.
	 */
	 ...
	 * @param y
	 *            the vertical component of the position
	 * @return an ArrayList (of FBody) of the 10 first bodies found at the given
	 *         position
	 */
	...
	public FBody raycastOne(float x1, float y1, float x2, float y2, FRaycastResult result, boolean solidShapes) {
		...
		// Redundantly do TestSegment a second time, as the previous one's results are
		// inaccessible
